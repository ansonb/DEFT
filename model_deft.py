    ...
    def forward(self,A_list, Nodes_list,nodes_mask_list, graph_list, A_list_u, pos_enc_list=[], return_filter_coeff_list=False):
        ...
        for unit in self.RT_layers:
            if return_filter_coeff_list:
                Nodes_list, filter_coeff_list = unit(A_list,A_list_u,graph_list,Nodes_list,return_filter_coeff_list=return_filter_coeff_list    )
            else:
                Nodes_list = unit(A_list,A_list_u,graph_list,Nodes_list)
...
class DEFTLayer(torch.nn.Module):
        ...
        if self.args.use_transformer:
            ...
            if self.args.aggregator=='GAT':
                ...
                if self.args.data.lower()=='reddit':
                    ...
                    residual = True
                delta = 2.5
                self.rgt_layer = dgl.nn.pytorch.conv.PNAConv(in_dim_rgt, out_dim_rgt, aggregators, scalers, delta, dropout=0.0, num_towers=1, edge_feat_size=0, residual=residual)
    ...
    def forward(self,A_list,A_list_u,G_list,node_embs_list,residual=True,return_filter_coeff_list=False):
